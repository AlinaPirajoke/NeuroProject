...
if 1:
    ...
    st.write('Свёрточные нейросети в основном используются для работы с изображениями, они нацелена на эффективное распознавание образов, изображенных на картинке. Если нужно понять, что за обьект изображён на фото, вы по адресу.')
    ...
    st.info("Код, представленный дальше, является шаблоном, вам необщодимо самим подставить необходимые значения при использовании. Для удобства можно открыть пример использования на моём примере про выдр.")
    st.subheader('Подготовка датасета')
    st.write("Для начала переместите датасет на ваше пространство Google Диск и откройте Colaboratory на том же аккаунте. Создайте новый блокнот. Для того, чтобы создать что-нибудь простенькое нам понадобится вот такой обьём инструментов: ")
...
!unzip / content / gdrive / "MyDrive" / Имя архиваированного файла с датасетом*''')
    ...
    st.write("В случае если у вас все фото в одной папке, то где-то неподалёку обязательно должен лежать csv файл с распределением этой кучи по классам, запомнике как в этом файле называются столбцы. Для того, чтобы решить проблему нам потребуется написать простенькую функцию, сортирующую фото")
